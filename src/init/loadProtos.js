...
export const loadProtos = async () => {
  try {
    ...
    for (const [packageName, types] of Object.entries(packetNames)) {
      ...
      for (const [typeName, messageName] of Object.entries(types)) {
        protoMessages[packageName][typeName] = root.lookupType(messageName);
      }
    }
  } catch (error) {
    throw new Error('Failed to load protobuf files: ' + error.message);
  }
};
